...
devtools::install_github("mkanai/corrplot", force = TRUE)
library(corrplot, R_LIBS_USER)
...
# diseases
TRAIT_CATEGORY2 = c(
  'education', 'reproductive', 'cognitive', 'anthropometric', 'cardiometabolic',
  'psychiatric', 'sleeping', 'aging', 'autoimmune', 'smoking_behaviour', 'haemotological'
)

################################################################################
args = commandArgs(trailingOnly=T)
if (identical(args, character(0))) {
  args = paste0("./input_example/", c("input_rg_hernias.txt", "traitlist_v2.txt"))
}
rg_fname = args[1]
...
corrplot_nsquare = function(rg, trait1, trait2, traits_use = NULL, order = "original", landscape=FALSE) {
  ...
  corrplot(mat2, method = "psquare", order = order,
           ...
           pch = "*", pch.cex = 1.5,
           na.label = "square", na.label.col = "grey30",full_col=FALSE)
  # return(list(mat = mat2, qmat = qmat2))
}


pdf("./output/corrplot_rg_disease.pdf", width = 16, height = 9, family = "Helvetica")
